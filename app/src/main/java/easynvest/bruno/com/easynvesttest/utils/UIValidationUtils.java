package easynvest.bruno.com.easynvesttest.utils;
        ...
        for(final EditText editText : editTexts){
            if(editText.getVisibility() == View.VISIBLE && (boolean)editText.getTag()) {
                ...
                if (editString.equals("")) {
                    if(til.getHint() != null){
                        Snackbar snackbar = UIUtils.createSnackbar(context, coordinatorLayout, til.getHint().toString() + " é obrigatório");
                        snackbar.show();
                    }
                ...
                if (editText.getInputType() == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS) {
                    if (!validateEmail(editString)) {
                        Snackbar snackbar = UIUtils.createSnackbar(context, coordinatorLayout, context.getString(R.string.email_invalido));
                        snackbar.show();
                ...
                if (editText.getInputType() == InputType.TYPE_CLASS_PHONE) {
                    if(editString.length() < 13 || editString.length() > 14){
                        Snackbar snackbar = UIUtils.createSnackbar(context, coordinatorLayout, context.getString(R.string.telefone_invalido));
                        snackbar.show();
